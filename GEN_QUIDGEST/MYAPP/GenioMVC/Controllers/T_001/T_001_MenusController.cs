...
namespace GenioMVC.Controllers
{
	...
	{
		private static readonly NavigationLocation ACTION_RMS_MENU_21 = new NavigationLocation("AGENTS29376", "RMS_Menu_21", "T_001") { vueRouteName = "menu-RMS_21" };
		private static readonly NavigationLocation ACTION_RMS_MENU_41 = new NavigationLocation("AGENTS29376", "RMS_Menu_41", "T_001") { vueRouteName = "menu-RMS_41" };


		//
		// GET: /T_001/RMS_Menu_21
...
// USE /[MANUAL RMS MENU_GET 21]/
			...
			if (querystring["ExportList"] != null && Convert.ToBoolean(querystring["ExportList"]) && querystring["ExportType"] != null)
			{
				...
				string file = "RMS_Menu_21_" + DateTime.Now.ToString("ddMMyyyyhhmmss") + "." + exportType;
				...
				byte[] fileBytes = null;
// USE /[MANUAL RMS OVERRQEXPORT 21]/
				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportList(listing, conditions, columns, exportType, file,ACTION_RMS_MENU_21.Name);
			...
			if (querystring["ImportList"] != null && Convert.ToBoolean(querystring["ImportList"]) && querystring["ImportType"] != null)
			{
				...
				string file = "RMS_Menu_21_Template" + "." + importType;
				List<CSGenio.framework.Exports.QColumn> columns = null;
				model.LoadToExportTemplate(out columns);
				byte[] fileBytes = null;

				fileBytes = new CSGenio.framework.Exports(UserContext.Current.User).ExportTemplate(columns, importType, file,ACTION_RMS_MENU_21.Name);
		...
		//
		// POST: /T_001/RMS_Menu_21_UploadFile
		[HttpPost]
		public ActionResult RMS_Menu_21_UploadFile(string importType, string qqfile) {
			RMS_Menu_21_ViewModel model = new RMS_Menu_21_ViewModel(UserContext.Current);

			PersistentSupport sp = UserContext.Current.PersistentSupport;
			List<CSGenioAt_001> rows = new List<CSGenioAt_001>();
			...
			try
			{
				...
				rows = new CSGenio.framework.Exports( UserContext.Current.User).ImportList<CSGenioAt_001>(columns, importType, fileBytes);

				sp.openTransaction();
				int lineNumber = 0;
				foreach (CSGenioAt_001 importRow in rows)
				{
					try
					{
						lineNumber++;
		...
		//
		// GET: /T_001/RMS_Menu_41
		[ActionName("RMS_Menu_41")]
		[HttpPost]
		public ActionResult RMS_Menu_41([FromBody]RequestMenuModel requestModel)
		{
			var queryParams = requestModel.QueryParams;
			...
			RMS_Menu_41_ViewModel model = new RMS_Menu_41_ViewModel(UserContext.Current);
			...
			bool isHomePage = RouteData.Values.ContainsKey("isHomePage") ? (bool)RouteData.Values["isHomePage"] : false;
			if (isHomePage)
				Navigation.SetValue("HomePage", "RMS_Menu_41");
			...
			if (!isHomePage &&
				(Navigation.CurrentLevel == null || !ACTION_RMS_MENU_41.IsSameAction(Navigation.CurrentLevel.Location)) &&
				Navigation.CurrentLevel.Location.Action != ACTION_RMS_MENU_41.Action)
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + Navigation.CurrentLevel.Location.ShortDescription());
			else if (isHomePage)
			{
				CSGenio.framework.Audit.registAction(UserContext.Current.User, Resources.Resources.MENU01948 + " " + ACTION_RMS_MENU_41.ShortDescription());
				Navigation.SetValue("HomePageContainsList", true);
			}



// USE /[MANUAL RMS MENU_GET 41]/
