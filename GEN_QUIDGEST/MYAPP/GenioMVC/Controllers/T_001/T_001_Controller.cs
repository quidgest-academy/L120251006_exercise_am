...
// USE /[MANUAL RMS MANUAL_CONTROLLER T_001]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.T_001 row = new Models.T_001(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("t_001");
			...
			try
			{
				...
				{
					case "FR_001__AT_01F_001___":	// Field (DB)
						{
							var model = new Fr_001_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Fr_001__at_01f_001___(qs);
							result = model.TableAt_01F_001;
						}
						break;
					case "FR_001__AT_02F_001___":	// Field (DB)
						{
							var model = new Fr_001_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Fr_001__at_02f_001___(qs);
							result = model.TableAt_02F_001;
						}
						break;
					default:
						break;
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "FR_001__AT_01F_001___":	// Field (DB)
						values = new Fr_001_ViewModel(UserContext.Current).GetDependant_Fr_001TableAt_01F_001(Selected);
						break;
					case "FR_001__AT_02F_001___":	// Field (DB)
						values = new Fr_001_ViewModel(UserContext.Current).GetDependant_Fr_001TableAt_02F_001(Selected);
						break;
					default: break;
				}
