<template>
	...
	<qtable 
		...
		<template #RMS="props">
			<q-badge
				v-for="userRole in props.row.UserRoles.filter(role => role.Module === 'RMS' && Modules[role.Module].active)"
				class="q-table__cell--multiple"
				:key="userRole.Role"
				variant="bold" >
				{{ Resources[userRole.Designation] }}
	...
	export default {
		...
		data() {
			...
			return {
				...
				userId: '',
				Modules: {
					RMS : { active: true, Cod: "RMS", Description: 'MY_APPLICATION56216'},
				},
				Users: {
					rows: [],
					total_rows: 0,
		...
		computed: {
			selectModules: {
                get() {
					...
					return                     this.Modules.RMS.active ;
                },
                set(value) {
                    //If we explicitly declare the variables, vue will bind them
                    this.Modules.RMS.active = value;
                    return value;
                }
            },
		...
		methods: {
			...
			userColumns() {
				...
				{
					label: () => vm.$t(vm.Modules.RMS.Description),
					name: "RMS",
					slot_name: "RMS",
					sort: false,
					visibility: vm.Modules.RMS.active && vm.columnModules
				},
			];
		},
		ImportarUsersAD() {
